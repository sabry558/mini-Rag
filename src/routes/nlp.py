...
async def get_project_index_info(request: Request, project_id: str):
    ...
    if index_info is None:
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"signal": ResponseSignal.FETCHING_INDEX_INFO_FAILED.value},
        )

    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={
            "signal": ResponseSignal.FETCHING_INDEX_INFO_SUCCESS.value,
...
async def search_index(
    ...
    results = nlp_controller.search_vector_db_collection(
        project=project, text=search_request.query_text, limit=search_request.limit
    )

    if not results:
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"signal": ResponseSignal.SEARCHING_INDEX_FAILED.value},
        )

    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={
            "signal": ResponseSignal.SEARCHING_INDEX_SUCCESS.value,
...
async def answer_rag_question(
    ...
    answer, full_prompt, chat_history = nlp_controller.answer_rag_question(
        project=project, question=search_request.query_text, limit=search_request.limit
    )

    if not answer:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"signal": ResponseSignal.ANSWERING_RAG_QUESTION_FAILED.value},
        )

    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={
            "signal": ResponseSignal.ANSWERING_RAG_QUESTION_SUCCESS.value,
            "answer": answer,
